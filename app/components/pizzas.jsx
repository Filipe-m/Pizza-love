...
import Pizzas from '../../database/pizzas.json'
...
function pizzas() {
  ...
  const pizzas = Pizzas.map(produto => (
    <li
      key={produto.id}
    ...
    >
      ...
      <h3 className=' font-bold font-Roboto text-lg ' >R${produto.valor[1]}</h3>
    </li>
  ))
  ...
  return (
    <div className="relative flex items-center mx-auto justify-center  w-[74%]  py-4 mt-3">
      <button
        className="absolute hidden md:grid place-items-center left-0 z-20 bg-bg-black dark:bg-primary w-8 h-8 rounded-full -translate-x-10  "
        onClick={left}
      >
        {' '}
        <HiOutlineArrowLeft className="stroke-red-600" />{' '}
      </button>
      <button
      ...
      <button
        className="absolute hidden md:grid place-items-center right-0 z-20 bg-bg-black dark:bg-primary w-8 h-8 rounded-full translate-x-10  "
        onClick={right}
      >
        {' '}
        <HiOutlineArrowRight className=" stroke-red-600" />{' '}
      </button>
      <button
